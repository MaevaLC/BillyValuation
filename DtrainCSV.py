...
csvf = open("DtrainV3.csv", "a") 
csva = open("DanswerV3.csv", "a")  
# load data
dataset = np.loadtxt("DtrainV3.csv", delimiter=",")
flatX_a = dataset[:,0:2730]
for flatXelement in flatX_a:
    for n in range(10):
        randomList = randBinList(30)
        delet = featureDelete(randomList, flatXelement)
        outF = csv.writer(csvf, delimiter=',', lineterminator = '\n')
        outF.writerow(delet)        
